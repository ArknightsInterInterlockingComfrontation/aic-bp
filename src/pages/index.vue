      ...
      <div :style="{ scale: bpViewData.scale }" class="h-[1080px] w-[1920px] origin-top-left overflow-hidden bg-[#3f3f46]">
        ...
        <div v-else class="absolute left-[337px] top-[236px] z-10 mx-auto h-[844px] w-[1250px] items-center bg-white p-2 pt-0">
          <div class="flex aspect-video w-full items-center justify-center overflow-hidden rounded-b-xl bg-zinc-700">
            <video ref="defaultPVVideo" autoplay v-show="videoSelector === 'default'" :src="defaultPV" class="h-full w-full object-cover" loop muted></video>
            <video ref="operatorPVVideo" @ended="operatorVideoEnded()" v-show="videoSelector === 'operator'" :src="operatorPV" class="h-full w-full object-cover" autoplay :muted="false"></video>
          </div>
          <div class="mt-2 flex h-[138px] w-full select-none items-center justify-center pb-[4px]">
            <div class="h-full w-full rounded-xl bg-zinc-700">
              <img class="mt-[4px] w-full" src="/img/底部信息.png" alt="" />
            </div>
          </div>
        </div>
...
const aple = function refreshVideoState() {
  if (!pausing.value) {
...
socket.on("NextTurn", function (data: any) {
  ...
  const targetTeam = states[data.turn - 1].team === "blue" ? data.teamData.blueTeam : data.teamData.redTeam;

  if (states[data.turn - 1].action === "ban" && targetTeam.ban.length > 1) {
    let selectedOperator = targetTeam.ban[targetTeam.ban.length - 1];
    if (selectedOperator !== "无") {
      operatorPV.value = `/pv/${selectedOperator}.mp4`;
      videoSelector.value = "operator";
      refreshVideoState();
    }
  } else if (states[data.turn - 1].action === "pick") {
    let selectedOperator = targetTeam.pick[targetTeam.pick.length - 1];
    if (selectedOperator !== "无") {
      if (isDone()) {
        let lastState = states[states.length - 1];
        if (lastState.team === "blue" && teamData.value.blueTeam.pick[9] !== "无") {
          operatorPV.value = `/pv/${teamData.value.blueTeam.pick[9]}.mp4`;
        }
        if (lastState.team === "red" && teamData.value.redTeam.pick[9] !== "无") {
          operatorPV.value = `/pv/${teamData.value.redTeam.pick[9] !== "无"}.mp4`;
        }
      } else {
        operatorPV.value = `/pv/${selectedOperator}.mp4`;
      }
...
socket.on("timer", function (timestamp) {
  ...
  if (document) {
    ...
    // document.documentElement.style.setProperty("--progress-start", `${clampedProgress/100}`);
